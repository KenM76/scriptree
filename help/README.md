...
- **`run_scriptreering.bat`** — the **cell + ring shell**: floating
  hexagonal launchers on your desktop, dock two together to form a
  multi-tool ring, click a cell to pop up its tool menu. See
...
hexagonal-cell launcher and how it plugs into the editor.
...
13. **[The cell + ring shell](cell_shell.md)** — V3's hexagonal cell
    launcher: gestures, ring docking, autostart, the
    `.scriptreering` file format. The shell calls the editor as a
    subprocess for every tool launch, so V1 stays untouched.
