...
Top-level entry point for the **cell + ring shell**.  Mirrors
``run_scriptree.py``'s prelude exactly (Python version check,
vendored ``lib/pypi/`` injection, ``SCRIPTREE_*`` env publishing,
package discovery walk) so anything that works for the editor's
bootstrap (portable Python, embeddable zip, PortableApps fallback)
works for the ring shell with no extra setup.  Then delegates to
``scriptree.shell.ring_main.main()`` instead of the V1 editor.

Usage::

    python run_scriptreering.py
    python run_scriptreering.py path/to/layout.scriptreering
    python run_scriptreering.py --register-autostart-user path/to/layout.scriptreering

Environment variables (same as ``run_scriptree.py``):
...
from scriptree.shell.ring_main import main  # noqa: E402
