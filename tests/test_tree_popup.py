...
# ---------------------------------------------------------------------------
# v0.6.5 — menu/tree icons
# ---------------------------------------------------------------------------

def _tool_with_embedded_icon(tmp_path: Path, name: str) -> Path:
    """A .scriptree carrying a tiny embedded PNG as its cell icon.

    The PNG is generated by Qt itself (a real 8x8 pixmap saved to
    disk) rather than a hand-pasted base64 blob — guarantees a
    valid, non-corrupt image regardless of platform libpng."""
    from PySide6.QtGui import QPixmap, QColor
    from scriptree.core.cell_metadata import embed_icon

    pm = QPixmap(8, 8)
    pm.fill(QColor("#3366cc"))
    icon_file = tmp_path / "ic.png"
    assert pm.save(str(icon_file), "PNG"), "Qt failed to write test PNG"
    p = _save_tool(tmp_path, name)
    embed_icon(str(p), str(icon_file))
    return p


def test_qicon_for_catalog_decodes_embedded_icon(
    tmp_path: Path,
) -> None:
    from scriptree.core.cell_metadata import qicon_for_catalog
    p = _tool_with_embedded_icon(tmp_path, "withicon")
    ic = qicon_for_catalog(str(p))
    assert ic is not None and not ic.isNull()
    # Cached: a second call returns the SAME object (mtime unchanged).
    assert qicon_for_catalog(str(p)) is ic


def test_qicon_for_catalog_none_when_no_icon(tmp_path: Path) -> None:
    from scriptree.core.cell_metadata import qicon_for_catalog
    p = _save_tool(tmp_path, "plain")  # no cell icon configured
    assert qicon_for_catalog(str(p)) is None  # cached None, no re-parse


def test_menu_leaf_actions_carry_icons(tmp_path: Path) -> None:
    a = _save_tool(tmp_path, "alpha")
    b = _tool_with_embedded_icon(tmp_path, "beta")
    tree = TreeDef(name="T", nodes=[
        TreeNode(type="folder", name="F", children=[
            TreeNode(type="leaf", name="alpha", path=a.name),
            TreeNode(type="leaf", name="beta", path=b.name),
        ]),
    ])
    out = tmp_path / "c.scriptreetree"
    save_tree(tree, out)
    menu = QMenu(None)
    _build_menu_for_catalog(menu, out)
    folder_action = menu.actions()[0]
    # Folder submenu has a (folder) icon.
    assert not folder_action.icon().isNull()
    sub = folder_action.menu()
    # Every leaf action has a non-null icon (custom or OS fallback).
