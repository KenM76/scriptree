...
class TestFastDragNoLeftBehind:
    """v0.8.0a77 (user-reported): dragging the forest QUICKLY to a
    screen edge left docked members behind with a gap (and overlapping)
    -- the per-frame ``_live_edge_reflow_or_fold`` relocated off-screen
    members WITHOUT updating their stored ``_members`` home, racing the
    rigid drag cascade (which shifts ``_members`` by the same delta).
    The reflow is removed; members follow rigidly (widget stays in sync
    with stored home) and the drag-end settle re-packs.
    """
    ...
    def _forest_with_member(self, tmp_path: Path, monkeypatch: Any):
        ...
        forest.move(500, 100)
        m.move(500, 200)
        forest._members[m._id] = QPoint(m.pos())
        forest._positioned.add(m._id)
        m._group_master_id = forest._id
        m._slot = ("inner", 3)
        return ctrl, forest, m

    def test_master_drag_does_not_run_live_edge_reflow(
        self, tmp_path: Path, monkeypatch: Any,
    ) -> None:
        from PySide6.QtCore import QPoint
        ...
        called: list = []
        monkeypatch.setattr(
            forest, "_live_edge_reflow_or_fold", lambda: called.append(1),
        )
        forest._drag_started = True
        forest._last_pos = QPoint(forest.pos())

        forest.move(forest.x(), forest.y() + 400)  # a master-drag step

        forest._drag_started = False
        assert called == [], (
            "the racy per-frame live-edge reflow must not run during a "
            "master drag (it caused the fast-drag left-behind/gap)"
        )
        m.close()

    def test_master_drag_keeps_widget_and_stored_home_in_sync(
        self, tmp_path: Path, monkeypatch: Any,
    ) -> None:
        from PySide6.QtCore import QPoint
        ...
        forest._last_pos = QPoint(forest.pos())
        m_start_y = m.pos().y()
        home_start_y = forest._members[m._id].y()

        forest.move(forest.x(), forest.y() + 300)  # rigid cascade

        forest._drag_started = False
        # The member widget follows the master rigidly...
        assert m.pos().y() == m_start_y + 300
        # ...and its stored home shifts by the SAME delta -- no
        # divergence, so it can't be "left behind" on the next move.
        assert forest._members[m._id].y() == home_start_y + 300
        m.close()
