...
class TestCascadeTranslateNestedMaster:
    ...
    with its own positioned dock-cluster, the cascade must reach the
    inner master's members so they tag along too.  Without this the
    user sees "forest moves; rings move; ring cells stay put — gap".
    """

    def test_inner_master_members_follow_outer_drag(self) -> None:
        from PySide6.QtCore import QPoint
        from scriptree.shell.branding_loader import load_branding
        from scriptree.shell.cell_window import CellWindow

        # Layout: outer master (forest) at (200, 200); inner master
        # (ring) docked at (256, 200) — one slot east of forest;
        # inner master has one cell at (312, 200) — one slot east of
        # the ring.
        forest = CellWindow(load_branding(), role="master")
        ring = CellWindow(load_branding(), role="master")
        cell = CellWindow(load_branding())
        for w in (forest, ring, cell):
            w.show()
        forest.move(200, 200)
        ring.move(256, 200)
        cell.move(312, 200)

        forest._members[ring._id] = QPoint(256, 200)
        forest._positioned.add(ring._id)
        ring._group_master_id = forest._id

        ring._members[cell._id] = QPoint(312, 200)
        ring._positioned.add(cell._id)
        cell._group_master_id = ring._id

        # Drive the cascade directly (the moveEvent path calls this
        # exact helper from inside the forest's drag-translate).
        forest._cascade_translate_positioned(100, 0)

        # All three widgets shifted by exactly +100 on x.  Inner
        # master's HOME slot for the cell shifted too so a future
        # collapse target tracks the new location.
        assert ring.pos().x() == 356
        assert cell.pos().x() == 412
        assert ring._members[cell._id].x() == 412

        forest.close()
        ring.close()
        cell.close()

    def test_cascade_is_reentry_safe(self) -> None:
        """The ``_GROUP_MOVE_IN_PROGRESS`` set must keep a master
        from re-entering its own translate.  Construction: a master
        whose ``_members`` happens to include its own id is
        pathological but cheap to guard against."""
        from PySide6.QtCore import QPoint
        from scriptree.shell.branding_loader import load_branding
        from scriptree.shell.cell_window import (
            CellWindow, _GROUP_MOVE_IN_PROGRESS,
        )

        master = CellWindow(load_branding(), role="master")
        master.show()
        master.move(100, 100)
        _GROUP_MOVE_IN_PROGRESS.add(master._id)
        try:
            # In-progress → cascade short-circuits.
            master._cascade_translate_positioned(50, 0)
            assert master.pos().x() == 100  # unchanged
        finally:
            _GROUP_MOVE_IN_PROGRESS.discard(master._id)
        master.close()
