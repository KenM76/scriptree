...
25. **Display-change resilience** (v0.8.0a26+ / a72) — change resolution, unplug a monitor, or remote-desktop in from a different-size machine while ScripTree is running, and every cell whose position is now off the visible work area is automatically pulled back on-screen (200 ms debounce on Qt's screen-change signals). Ring members are repacked together around their master rather than stacking at the screen edge. Forest right-click → **Bring all cells back on-screen** is the manual escape hatch when the automatic rescue misses anything.
...
29. **Collapse/expand keeps cells on-screen and non-overlapping** (v0.8.0a68) — single-clicking the forest hub or a ring master to collapse tucks members into the hub; clicking again expands them through the same free-slot placement engine that startup uses. Cells always bloom on a free, on-screen, non-overlapping honeycomb slot attached to the hub's current position. Moving the hub while collapsed and then expanding puts cells around the new position, not the old one.

30. **Hub always returns to a visible screen** (v0.8.0a69) — if the forest was last used with a monitor that is now absent, or saved at a position that no longer fits the current display, the hub is automatically clamped back onto a visible screen the next time it is revealed (tray click, taskbar restore, or application launch). The "forest disappeared" symptom from a stale saved position is eliminated.
